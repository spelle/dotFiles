...
function run_once_scr(prg,arg_string,pname,screen)
    if not prg then
        do return nil end
    end
...
run_once_scr("empathy",nil,nil,2)
run_once_scr("skype",nil,nil,2)
run_once_scr("xchat",nil,nil,2)
...
-- beautiful init
beautiful.init(os.getenv("HOME") .. "/.config/awesome/themes/ecrins/theme.lua")
...
browser         = "dwb"
browser_private = "google-chrome --incognito"
files = "nautilus --browser --no-desktop"
gui_editor = "gvim"
graphics   = "gimp"
...
-- {{{ Tags
tags = {
   names = { ".:: #1: Internet", ":: #2: Mail", ":: #3: Messaging", ":: #4: Terminal(s)", ":: #5: Development", ":: #6: Files", ":: #7: Media(s)", ":: #8: Other ::." },
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey }, "i", function () awful.util.spawn(browser_private) end),
...
awful.rules.rules = {
    ...
    { rule = { class = "URxvt" },
          properties = { opacity = 0.99, tag = tags[1][4] } },
